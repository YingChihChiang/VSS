...
static void scale_vector(double *v, double s) {
  v[0] *= s;
  v[1] *= s;
  v[2] *= s;
}

static void scale_add_vector(double *c, double s, const double *v) {
  c[0] += s * v[0];
  c[1] += s * v[1];
  c[2] += s * v[2];
}

static void copy_vector(double *c, const double *v) {
  c[0] = v[0];
  c[1] = v[1];
  c[2] = v[2];
}
...
void reciprocal_lattice(const double *cell, double *rcell) {
  const double *a1, *a2, *a3;
  double *b1, *b2, *b3;
...
int fill_charges(const int *dims, const double *cell, int natoms,
                const double *xyzq, double *q_arr, double *rcell, double *oddd) {             

  int i, j, k, l;
  int K1, K2, K3, dim2, dim3;
  double frac[3], Mi[12];
  double ox,oy,oz,r1x,r1y,r1z,r2x,r2y,r2z,r3x,r3y,r3z,kx,ky,kz;

  K1=dims[0]; K2=dims[1]; K3=dims[2]; dim2=dims[3]; dim3=dims[4];

  memset( (void*) q_arr, 0, K1*dim2*dim3 * sizeof(double) );
